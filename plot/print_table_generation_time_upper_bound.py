"""
LaTeX tables: Instance UB (fix_w_per_block) computation time in hours (1 decimal).

Same grid as print_table_mopt_upper_bound / print_table_generation_time:
  results/upper_bound/fix_w_per_block/{dataset}/[seed0/]lambda{L}/epsilon{E}/upper_bound.json
  → computation_time_sec (C++ Timer in compute_upper_bound).

Outputs: fig/table_generation_time_upper_bound/time_ub_full_lambda{0p01,0p1}.tex
"""
from __future__ import annotations

import argparse
import math
import sys
from pathlib import Path
from typing import Dict, List, Optional, Tuple

from load_results_upper_bound import load_upper_bound_results
from plot_config import (
    LAMBDA_TABLE_SPEC_1M_DUPLICATES,
    LAMBDA_TABLE_SPEC_FULL_DUPLICATES,
    dataset_display_name,
    upper_bound_dataset_specs,
    upper_bound_dataset_specs_duplicate_only,
    upper_bound_dataset_specs_full,
    upper_bound_dataset_specs_full_duplicate_only,
)

from print_table_mopt_upper_bound import (
    LAMBDA_TABLE_SPEC_1M,
    LAMBDA_TABLE_SPEC_FULL,
    TABLE_EPSILONS,
    TABLE_LAMBDA_FRACS,
    _LATEX_ROW_END,
)

DEFAULT_UB_BASE = "results/upper_bound/fix_w_per_block"
DEFAULT_SEED = 0
DEFAULT_OUT_DIR = Path("fig/table_generation_time_upper_bound")
DEFAULT_OUT_DIR_DUP = Path("fig/table_generation_time_upper_bound_dup")


def _latex_dataset_name(dataset_id: str) -> str:
    return dataset_display_name(dataset_id).replace("_", r"\_")


def _instance_ub_computation_sec(
    ub_path: str,
    epsilon: int,
    seed: int,
    lambda_val: int,
) -> Optional[float]:
    """computation_time_sec for one (upper_bound dir, λ, ε, seed)."""
    df = load_upper_bound_results(ub_path, epsilon, seed)
    if df.empty:
        return None
    sub = df[df["lambda"] == int(lambda_val)]
    if sub.empty:
        return None
    v = sub.iloc[0].get("computation_time_sec")
    if v is None:
        return None
    try:
        x = float(v)
    except (TypeError, ValueError):
        return None
    if isinstance(x, float) and math.isnan(x):
        return None
    if not math.isfinite(x) or x < 0:
        return None
    return x


def _hours_one_decimal(sec: Optional[float]) -> str:
    if sec is None:
        return r"\dots"
    h = sec / 3600.0
    return f"{h:.1f}"


def format_latex_ub_time_stacked(
    lambda_frac: str,
    print_label: str,
    resolution_lines: List[str],
    epsilon_blocks: List[Tuple[int, List[Tuple[str, str]]]],
) -> str:
    """ε | Dataset | Time (h); no multirow (ε repeated each row)."""
    if not epsilon_blocks:
        return ""

    n_ds = len(epsilon_blocks[0][1])
    lines: List[str] = [
        f"% Instance UB computation time (h, 1 decimal) — {print_label}, "
        f"$\\lambda={lambda_frac}\\,n$. "
        f"Source: upper_bound.json \\texttt{{computation\\_time\\_sec}} / 3600 "
        f"(fix\\_w\\_per\\_block, same as table\\_mopt\\_upper bound Instance UB).",
        r"% \usepackage{booktabs}",
        *resolution_lines,
        r"\begin{tabular}{@{}c l r@{}}",
        r"\toprule",
        " & & "
        + rf"\multicolumn{{1}}{{c}}{{$\lambda = {lambda_frac}\,n$ (Instance UB)}} \\",
        r"\cmidrule(lr){3-3}",
        r"$\varepsilon$ & Dataset & Time (h) " + _LATEX_ROW_END,
        r"\midrule",
    ]

    n_eps = len(epsilon_blocks)
    for b_idx, (eps, block_rows) in enumerate(epsilon_blocks):
        if len(block_rows) != n_ds:
            raise ValueError(
                f"Inconsistent row count for ε={eps}: expected {n_ds}, got {len(block_rows)}"
            )
        for name, hour_cell in block_rows:
            lines.append(f"{eps} & {name} & {hour_cell}" + _LATEX_ROW_END)
        if b_idx < n_eps - 1:
            lines.append(r"\midrule")

    lines.append(r"\bottomrule")
    lines.append(r"\end{tabular}")
    return "\n".join(lines)


def write_ub_time_tables(
    *,
    ub_base: str,
    seed: int,
    out_dir: Path,
    log_append: Optional[Path] = None,
    variants: Tuple[str, ...] = ("full",),
    duplicate_only: bool = False,
) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    if log_append is not None:
        log_append.parent.mkdir(parents=True, exist_ok=True)

    dup_suffix = "_dup" if duplicate_only else ""

    for variant in variants:
        if variant == "full":
            datasets = (
                upper_bound_dataset_specs_full_duplicate_only()
                if duplicate_only
                else upper_bound_dataset_specs_full()
            )
            lambda_table_spec = (
                LAMBDA_TABLE_SPEC_FULL_DUPLICATES
                if duplicate_only
                else LAMBDA_TABLE_SPEC_FULL
            )
            print_label = "full"
        elif variant == "1m":
            datasets = (
                upper_bound_dataset_specs_duplicate_only()
                if duplicate_only
                else upper_bound_dataset_specs()
            )
            lambda_table_spec = (
                LAMBDA_TABLE_SPEC_1M_DUPLICATES
                if duplicate_only
                else LAMBDA_TABLE_SPEC_1M
            )
            print_label = "1M"
        else:
            raise ValueError(f"unknown variant: {variant}")

        for lambda_frac in TABLE_LAMBDA_FRACS:
            resolution_lines: List[str] = []
            for d in datasets:
                name = d["name"]
                lam = lambda_table_spec[lambda_frac][name]
                resolution_lines.append(f"% {name}: λ = {lam} (f={lambda_frac})")

            epsilon_blocks: List[Tuple[int, List[Tuple[str, str]]]] = []
            for epsilon in TABLE_EPSILONS:
                block_rows: List[Tuple[str, str]] = []
                for d in datasets:
                    ub_path = f"{ub_base}/{d['path']}"
                    lam = lambda_table_spec[lambda_frac][d["name"]]
                    sec = _instance_ub_computation_sec(ub_path, epsilon, seed, lam)
                    block_rows.append(
                        (
                            _latex_dataset_name(d["name"]),
                            _hours_one_decimal(sec),
                        )
                    )
                epsilon_blocks.append((epsilon, block_rows))

            body = format_latex_ub_time_stacked(
                lambda_frac,
                print_label,
                resolution_lines,
                epsilon_blocks,
            )
            suffix = lambda_frac.replace(".", "p")
            output = out_dir / f"time_ub_{variant}_lambda{suffix}{dup_suffix}.tex"
            output.write_text(body + "\n", encoding="utf-8")
            msg = f"Wrote LaTeX table: {output}"
            print(msg, file=sys.stderr)
            if log_append is not None:
                with open(log_append, "a", encoding="utf-8") as f:
                    f.write(msg + "\n")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="LaTeX tables for Instance UB (fix_w_per_block) computation time (hours)."
    )
    parser.add_argument(
        "--ub-base",
        default=DEFAULT_UB_BASE,
        help="Base dir for fix_w_per_block upper bound results",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=DEFAULT_SEED,
        help="seed{S} when present under dataset (default: 0)",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=DEFAULT_OUT_DIR,
        help="Output directory for .tex files",
    )
    parser.add_argument(
        "--log-append",
        type=Path,
        default=None,
        help="Optional log file to append messages",
    )
    parser.add_argument(
        "--duplicate-only",
        action="store_true",
        help="Duplicate-key datasets only; *_dup.tex under table_generation_time_upper_bound_dup by default.",
    )
    args = parser.parse_args()

    out_dir = args.out_dir
    if args.duplicate_only and args.out_dir == DEFAULT_OUT_DIR:
        out_dir = DEFAULT_OUT_DIR_DUP

    write_ub_time_tables(
        ub_base=args.ub_base,
        seed=args.seed,
        out_dir=out_dir,
        log_append=args.log_append,
        duplicate_only=args.duplicate_only,
    )


if __name__ == "__main__":
    main()
